...
from models import HINER, HINERDecoder


def main():
    parser = argparse.ArgumentParser()
    # Dataset parameters
    parser.add_argument("--data_path", type=str, default="", help="data path for the HSI")
    parser.add_argument(
        "--vid", type=str, default="hsi", help="HSI identifier for output naming"
    )
    parser.add_argument(
        "--data_split",
        type=str,
        default="1_1_1",
        help="valid_train/total_train/all band split",
    )
    parser.add_argument(
        "--ori_shape",
        type=str,
        default="",
        help="original H_W_C, used when dumping reconstructions",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="1_1",
        help="block number for encoer and decoder",
    )
    parser.add_argument(
        "--norm",
        default="none",
    ...
    # HINER: encoder is fixed (positional encoding + MLP); decoder is pixelshuffle
    args.conv_type = ["pe+mlp", "pshuffel"]
    ...
    exp_id = (
        ...
        + f"_e{args.epochs}_b{args.batchSize}_{args.quant_str}_lr{args.lr}_{args.lr_type}_{args.loss}_{extra_str}{args.act}{args.suffix}"
    )
    args.exp_id = exp_id
...
def train(local_rank, args):
    ...
    train_ind_list, args.val_ind_list = data_split(
        list(range(args.full_data_length)), split_num_list, False, 0
    )
    ...
    embed_param = (
        float(embed_dim) / total_enc_strds**2 * args.final_size * args.full_data_length
    )
    ...
    fix_ch_stages = len(args.dec_strds)
    a = ch_reduce * sum(
        [
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for i, sample in enumerate(train_dataloader):
            ...
            img_data, img_gt, inpaint_mask = img_data, img_data, torch.ones_like(img_data)
...
def Dump2CSV(args, best_results_list, results_list, psnr_list, filename="results.csv"):
    ...
    result_dict.update(
        {
            f"{k}": RoundTensor(v, 2 if "psnr" in k else 4)
...
def evaluate(
    ...
    for model_ind, cur_model in enumerate(model_list):
        ...
        for i, sample in enumerate(full_dataloader):
            ...
            if i > 10 and args.debug:
                break
            img_data, img_gt, _ = img_data, img_data, torch.ones_like(img_data)
            img_out, embed_list, dec_time = cur_model(
                img_data, norm_idx, input_embed=1 if model_ind else None
            )
            if model_ind == 0:
            ...
            if dump_vis:
                for batch_ind, cur_img_idx in enumerate(img_idx):
                    ...
                    cv2.imwrite(
                        f"{visual_dir}/pred_{img_ind:04d}_{temp_psnr_list}.png",
                        (data_png.squeeze() * 65535.0 / 255.0),
                    )
                    ...
                    img = center_crop(
                        img_out[batch_ind].cpu().squeeze(0), (ori_shape_h, ori_shape_w)
                    )
                    img = img.numpy()
        ...
        if dump_vis:
            ...
            sio.savemat(
                f"{final_mat_path}/{args.quant_model_bit}_final.mat", {"hsi": final_mat}
            )
    ...
    if local_rank in [0, None] and quant_ckt != None:
        ...
        torch.jit.save(
            torch.jit.trace(HINERDecoder(model), (vid_embed[:2])),
