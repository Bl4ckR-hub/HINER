...
def main():
    ...
    parser.add_argument('--sr_ratio', type=int, default=2, help='data SR')
    ...
    exp_id = f'{args.vid}_spectraSR_{args.sr_ratio}/{args.data_split}_{embed_str}_FC{args.fc_hw}_KS{args.ks}_RED{args.reduce}_low{args.lower_width}_blk{args.num_blks}' + \
...
def train(local_rank, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for i, sample in enumerate(train_dataloader):
            ...
            final_loss = loss_fn(img_out, img_data, args.loss)# + 0.01 * SAM.mean()    
            optimizer.zero_grad()
            final_loss.backward()
            optimizer.step()
...
def evaluate(model, full_dataloader, local_rank, args, dump_vis=False):
    ...
    for i, sample in enumerate(full_dataloader):
        ...
        if i > 10 and args.debug:
            break
        img_out, embed_list, dec_time = model(img_data, norm_idx, img_idx, spectraSR=args.sr_ratio if img_idx in args.val_ind_list else -1)
        ...
        if i % args.print_freq == 0 or i == len(full_dataloader) - 1:
            ...
            for v_name, v_list in zip(args.metric_names, metric_list):
                ...
                elif 'pred_unseen_psnr' in v_name:
                    psnr = torch.stack(v_list, dim=-1).mean(-1) if len(v_list) else torch.zeros(1)
                    print_str += f'{v_name}: {RoundTensor(psnr, 2)} | '
            if local_rank in [0, None]:
                print(print_str, flush=True)
