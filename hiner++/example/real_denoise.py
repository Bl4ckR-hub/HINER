...
from data import HSIDataSet, NoisedHSIDataSet
from models import HINER, HINER2


def main():
    parser = argparse.ArgumentParser()
    # Dataset parameters
    parser.add_argument('--clean_data_path', type=str, default='', help='data path for vid',)
    parser.add_argument('--noise_data_path', type=str, default='', help='data path for vid',)
    ...
    exp_id = f'{args.vid}_real_denoise/{args.data_split}_{embed_str}_FC{args.fc_hw}_KS{args.ks}_RED{args.reduce}_low{args.lower_width}_blk{args.num_blks}' + \
...
def train(local_rank, args):
    ...
    # setup dataloader
    full_dataset = NoisedHSIDataSet(args)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for i, sample in enumerate(train_dataloader):
            clean_img, noise_img, norm_idx, img_idx = data_to_gpu(sample['clean_img'], device), data_to_gpu(sample['noise_img'], device), data_to_gpu(sample['norm_idx'], device), data_to_gpu(sample['idx'], device)
            ...
            img_out, _, _ = model(noise_img, norm_idx, img_idx)

            final_loss = loss_fn(img_out, noise_img, args.loss)# + 0.01 * SAM.mean()    
            optimizer.zero_grad()
            final_loss.backward()
            optimizer.step()
            ...
            pred_psnr_list.append(psnr_fn_single(img_out.detach(), clean_img)) 
...
def evaluate(model, full_dataloader, local_rank, args, 
    ...
    for i, sample in enumerate(full_dataloader):
        ...
        if i > 10 and args.debug:
            break
        img_out, embed_list, dec_time = model(noise_img, norm_idx, img_idx)
        ...
        if args.eval_fps:
            time_list.pop()
            for _ in range(100):
                img_out, embed_list, dec_time = model(noise_img, norm_idx, img_idx, embed_list[0])
                time_list.append(dec_time)

        pred_psnr = psnr_fn_batch([img_out], clean_img)
        ...
        if dump_vis:
            for batch_ind, cur_img_idx in enumerate(img_idx):
                ...
                dump_img_list = [noise_img[batch_ind], img_out[batch_ind]]
