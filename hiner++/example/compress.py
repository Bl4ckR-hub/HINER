...
def main():
    ...
    exp_id = f'{args.vid}_compression/{args.data_split}_{embed_str}_FC{args.fc_hw}_KS{args.ks}_RED{args.reduce}_low{args.lower_width}_blk{args.num_blks}' + \
...
def train(local_rank, args):
    ...
    print_str = '======================Full-precision model========================'
    print(print_str, flush=True)
    print_str = 'Evaluation ... \n {} \n'.format(datetime.now().strftime('%Y_%m_%d_%H_%M_%S'))
    results_list, hw, embedding_list = evaluate(model, full_dataloader, local_rank, args, args.dump_vis)
    ...
    wq_params = {'n_bits': 6, 'channel_wise': True, 'scale_method': 'MinMax'}
    ...
    print('input embedding shape: {}'.format(cali_data.shape))
    ...
    for i, sample in enumerate(full_dataloader):
        ...
        _, _, _ = qnn(img_data, norm_idx, img_idx)
        break
    init_time = time.time() - init_start
    print('Init time: {}'.format(init_time))
    
    print_str = '======================Close quantization model========================'
    ...
    if local_rank in [0, None]:
        ...
        with open('{}/eval.txt'.format(args.outf), 'a') as f:
            f.write(print_str + '\n\n')   
    print_str = 'Evaluation ... \n {} \n'.format(datetime.now().strftime('%Y_%m_%d_%H_%M_%S'))
    results_list, _, _ = evaluate(model, full_dataloader, local_rank, args, args.dump_vis)
    for i, (metric_name, metric_value) in enumerate(zip(args.metric_names, results_list)):
        cur_v = RoundTensor(metric_value.max(), 2 if 'psnr' in metric_name else 4)
        print_str += f'best_{metric_name}: {cur_v} | '
    if local_rank in [0, None]:
    ...
    print_str = '======================Weight quantization model w/o opt========================'
    qnn.set_quant_state(True)
    if local_rank in [0, None]:
        print(print_str, flush=True)
        with open('{}/eval.txt'.format(args.outf), 'a') as f:
    ...
    kwargs = dict(gt=train_dataloader, 
                  outf=args.outf,
                  loss=args.loss,
                  iters=5000, 
                  weight=0.01,
    ...
    bits = qnn.compute_model_bits()
    args.bpp = bits / args.full_data_length / args.final_size
    print_str = '=================bit rate: {}================='.format(args.bpp)
    if local_rank in [0, None]:
        print(print_str, flush=True)
        with open('{}/eval.txt'.format(args.outf), 'a') as f:
            f.write(print_str + '\n\n')   
    ...
    print_str = '======================Weight quantization model w/ opt========================'
    qnn.set_quant_state(True)
    if local_rank in [0, None]:
        print(print_str, flush=True)
        with open('{}/eval.txt'.format(args.outf), 'a') as f:
    ...
    full_bpp = huffman_coding(args, cali_data, qnn)
    print_str = '=================Full bpp:\t{:.4f}================='.format(float(full_bpp))
    if local_rank in [0, None]:
        print(print_str, flush=True)
        with open('{}/eval.txt'.format(args.outf), 'a') as f:
            f.write(print_str + '\n\n')   
...
def evaluate(model, full_dataloader, local_rank, args, dump_vis=False):
    ...
    for i, sample in enumerate(full_dataloader):
        ...
        pred_psnr = psnr_fn_batch([img_out], img_data)
