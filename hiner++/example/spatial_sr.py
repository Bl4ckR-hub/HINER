...
def main():
    ...
    args.sr_ratio = int(args.restore.split('_')[-1])
...
def pad_to_match_spatial(img, target_hw):
    """Center-pad a CHW tensor so visualizations can be concatenated safely."""
    target_h, target_w = target_hw
    h, w = img.shape[-2:]
    if h == target_h and w == target_w:
        return img
    if h > target_h or w > target_w:
        return center_crop(img, (target_h, target_w))
...
def train(local_rank, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for i, sample in enumerate(train_dataloader):
            ...
            img_out, _, _ = model(img_data, norm_idx, img_idx, spatialSR=args.sr_ratio)
            _img_out = torch.nn.functional.interpolate(img_out, scale_factor=1./args.sr_ratio, mode='bicubic').clamp_(0,1)
            final_loss = loss_fn(_img_out, img_data, args.loss)# + 0.01 * SAM.mean()    
            optimizer.zero_grad()
            final_loss.backward()
            optimizer.step()
...
def evaluate(model, full_dataloader, local_rank, args, dump_vis=False):
    ...
    for i, sample in enumerate(full_dataloader):
        ...
        if i > 10 and args.debug:
            break
        img_data, img_gt, inpaint_mask = args.transform_func(img_data)
        img_out, embed_list, dec_time = model(img_data, norm_idx, img_idx, spatialSR=args.sr_ratio)
        ...
        if args.eval_fps:
            time_list.pop()
            for _ in range(100):
                img_out, embed_list, dec_time = model(img_data, norm_idx, img_idx, embed_list[0], False, spatialSR=args.sr_ratio)
                time_list.append(dec_time)

        # _img_data = torch.nn.functional.interpolate(img_data, scale_factor=2, mode='bicubic')
        ...
        if dump_vis:
            for batch_ind, cur_img_idx in enumerate(img_idx):
                ...
                vis_input = pad_to_match_spatial(img_data[batch_ind], img_out[batch_ind].shape[-2:])
                dump_img_list = [vis_input, img_out[batch_ind]]
